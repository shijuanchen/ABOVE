...
def post_process_smoothing(tile_name, combine_folder, category_folder, lc_folder, output_folder):
    ...
    for year in year_avail:
        ...
        for i in np.arange(k, nrows-k):
            for j in np.arange(k,ncols-k):  
                ...
                # if the pixel is FN insects, FN logging or FN others
                    ...
                    if abs(int(fdb_array[i, j]) + 1792 - year) == 0:
                    ...
                    elif abs(int(fdb_array[i, j]) + 1792 - year) > 0:
                        window_s = np.array(cc_array[i-ks:i+ks+1, j-ks:j+ks+1])
                        window_s = np.reshape(window_s, ns * ns)
                        cc_win_FN_no_fire = []
                        for pix in window_s:
        ...
        outfile = output_folder+'/'+tile_name+'_FF_FN_NF_NN_' + str(year) +'_cl_' + str(window_size) + '_sm_v3.tif'
...
def main(tile_name):
    ...
    output_folder = r'/projectnb/landsat/users/shijuan/above/post_process/smooth_v3/{0}'.format(tile_name)
