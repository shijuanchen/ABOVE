...
def post_process_fire_filter(tile_name, combine_folder, category_folder, output_folder, window_size):
    ...
    for year in year_avail:
        ...
        outfile = output_folder+'/'+tile_name+'_FF_FN_NF_NN_' + str(year) +'_cl_' + str(window_size) + '_sm_v2.tif'
...
def main(tile_name, window_size):
    ...
    output_folder = r'/projectnb/landsat/users/shijuan/above/post_process/smooth_v2/{0}'.format(tile_name)
    post_process_fire_filter(tile_name, combine_folder, category_folder, output_folder, window_size)
